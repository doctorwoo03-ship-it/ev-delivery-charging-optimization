...
import {
  vehicle,
  depot,
  deliveries as sampleDeliveries,
  chargingStations,
  recommendation,
...
function MVP2Page() {
  ...
  const handleAddDelivery = (newDelivery) => {
    if (mapRef.current && window.kakao) {
      const pos = new window.kakao.maps.LatLng(newDelivery.lat, newDelivery.lng)
      ...
      el.addEventListener('click', () => {
        if (infoWindowRef.current) {
          infoWindowRef.current.setContent(
            `<div style="padding:8px;font-size:13px;">${newDelivery.name}</div>`
          )
      ...
      markersRef.current[newDelivery.id] = overlay
    }
    ...
    script.onload = () => {
      ...
      window.kakao.maps.load(() => {
        ...
        sampleDeliveries.forEach((d) => {
          ...
          overlay.setMap(map)
          markersRef.current[d.id] = overlay
        })
